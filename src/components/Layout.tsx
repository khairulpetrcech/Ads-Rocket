  ...
  return (
    <div className="flex h-screen bg-white text-slate-800 overflow-hidden relative">
        ...
        <div className="px-5 mb-2">
            <div className="relative">
                ...
                <input 
                    type="text" 
                    placeholder="Search" 
                    className="w-full bg-slate-50 border border-slate-200 rounded-lg pl-9 pr-3 py-2 text-xs text-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
                />
            </div>
        </div>
            ...
            <div className="bg-white rounded-lg p-3 mb-3 border border-slate-200 shadow-sm">
                <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2 text-indigo-600">
                        <Zap size={14} className="fill-indigo-600" /> <span className="text-[10px] font-bold uppercase">AI Status</span>
                    </div>
                    <button onClick={() => setIsAiExpanded(!isAiExpanded)} className="text-slate-400 hover:text-slate-600">
                        {isAiExpanded ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                    </button>
                </div>
                {isAiExpanded && (
                    <div className="animate-fadeIn">
                        {loadingAi ? <div className="text-xs text-slate-500 py-1 flex gap-2"><Loader2 size={12} className="animate-spin"/> Analyzing...</div> : (
                            <ul className="text-[10px] text-slate-500 space-y-1 list-disc pl-3 mb-2">
                                {aiStatus.length > 0 ? aiStatus.slice(0, 2).map((plan, i) => <li key={i}>{plan}</li>) : <li className="list-none italic text-slate-400">Ready.</li>}
                            </ul>
                        )}
                        <button onClick={handleAnalyze} disabled={loadingAi} className="w-full flex items-center justify-center gap-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-600 text-[10px] font-bold py-1.5 rounded border border-indigo-100 transition-colors">
      ...
      <main className="flex-1 overflow-y-auto relative pt-16 md:pt-0 bg-[#f8fafc]">
        {/* Top Breadcrumb Bar for Desktop */}
        <div className="hidden md:flex items-center justify-between px-6 py-3 bg-white border-b border-slate-200 sticky top-0 z-30">
            <div className="flex items-center gap-2 text-sm">
                <span className="font-semibold text-slate-700">Const Office</span>
                <span className="text-slate-400">/</span>
                <span className="text-slate-500 font-medium">Dashboard</span>
            </div>
            <div className="flex items-center gap-4">
               <button className="text-slate-400 hover:text-indigo-600 transition-colors"><HelpCircle size={18}/></button>
